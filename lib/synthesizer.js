        ...
        allResults.forEach(r => {
            if (r.snippet) {
                const sents = r.snippet.split(/(?<=[.!?])\s+/).filter(s => {
                    ...
                    if (t.match(/^\d{4}\s/)) return false; // starts with year
                    if (t.match(/\(\d{4}\)/)) return false; // has (year)
                    if (t.match(/^[A-Z][a-z]+ [A-Z]/) && t.length < 60) return false; // author name
                    if (t.match(/doi:|pmid:|pubmed/i)) return false;
                    if (t.match(/^[A-Z][a-z]+, [A-Z]/)) return false; // Last, First format
                    if (t.match(/^[A-Z][a-z]+ [A-Z][a-z]+$/)) return false; // two words = author
                    return true;
                });
                sents.forEach(s => {
